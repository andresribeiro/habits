...
export default function HabitsList({
	...
	async function handleToggleHabit(habitId: string) {
		...
		setHabitsInfo({
			possible_habits: habitsInfo!.possible_habits,
		...
		<div className="mt-6 flex flex-col gap-3">
			{habitsInfo?.possible_habits.map((habit) => (
				<Checkbox.Root
					...
					checked={habitsInfo.completed_habits.includes(habit.id)}
				...
				>
					...
					<span className="font-semibold text-xl test-white leading-tight group-data-[state=checked]:line-through group-data-[state=checked]:text-zinc-400">
						{habit.title}
					</span>
				</Checkbox.Root>
			))}
