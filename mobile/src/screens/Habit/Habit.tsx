...
export default function Habit() {
	...
	const habitsProgress = dayInfo?.possible_habits.length
		? generateProgressPercentage(
				dayInfo?.possible_habits.length,
		...
		try {
			...
			if (completedHabits.includes(habitId)) {
				setCompletedHabits((previousCompletedHabits) =>
					previousCompletedHabits.filter((habit) => habit !== habitId),
				);
			} else {
				setCompletedHabits((previousCompletedHabits) => [
					...previousCompletedHabits,
					habitId,
				]);
			}
		} catch {
	...
	return (
		<View className="flex-1 bg-background px-8 pt-16">
			...
			>
				...
				>
					{dayInfo ? (
						dayInfo.possible_habits.map((habit) => (
							<Checkbox
								key={habit.id}
								title={habit.title}
